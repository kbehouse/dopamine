...
from dopamine.agents.rainbow import rainbow_siamese_agent
...
def create_fetch_cam_environment():
  ...
  env = FetchDiscreteCamSiamenseEnv(dis_tolerance = 0.001, 
                                    step_ds=0.005, 
                                    gray_img=False, 
                                    hsv_color=FLAGS.hsv,
                                    is_render=True)
...
def launch_experiment(create_runner_fn, create_agent_fn):
  ...
  Args:
    ...
    create_agent_fn: A function that takes as args a Tensorflow session and an
     Atari 2600 Gym environment, and returns an agent.
  """
  run_experiment_siamese.load_gin_configs(FLAGS.gin_files, FLAGS.gin_bindings)
