...
from dopamine.agents.rainbow import rainbow_curiosity_agent
from dopamine.fetch_cam_train import run_experiment_rgb
...
def create_agent(sess, environment, summary_writer=None):
  return rainbow_curiosity_agent.RainbowCuriosityAgent(
        sess, num_actions=6,
...
def create_runner(base_dir, create_agent_fn):
  ...
  # Continuously runs training until max num_iterations is hit.
    ...
    return run_experiment_rgb.TrainRunner(base_dir, create_agent_fn)
  else:
...
def launch_experiment(create_runner_fn, create_agent_fn):
  ...
  Args:
    ...
    create_agent_fn: A function that takes as args a Tensorflow session and an
     Atari 2600 Gym environment, and returns an agent.
  """
  run_experiment_rgb.load_gin_configs(FLAGS.gin_files, FLAGS.gin_bindings)
