...
class Runner(object):
  ...
  def _run_one_step(self, action):
    ...
    Returns:
      The observation, reward, and is_terminal values returned from the
        environment.
    """
    gripper = 1 if  self._environment.is_gripper_close else 0 
    observation, reward, is_terminal, _ = self._environment.step(action)
    return observation, reward, is_terminal, gripper

  def _end_episode(self, reward, gripper):
    """Finalizes an episode run.

    Args:
      reward: float, the last reward from the environment.
    """
    self._agent.end_episode(reward, gripper)
  ...
  def _run_one_episode(self):
    ...
    while True:
      observation, reward, is_terminal, is_gripper_close = self._run_one_step(action)
      ...
      if (is_terminal or #self._environment.game_over or 
        ...
        action = self._agent.step(reward, observation, is_gripper_close)

    self._end_episode(reward, is_gripper_close)
