...
from dopamine.agents.rainbow import rainbow_rgb_gripper_agent
from dopamine.fetch_cam_train import run_experiment_rgb_gripper
...
def create_runner(base_dir, create_agent_fn):
  ...
  if FLAGS.schedule == 'continuous_train_and_eval':
    ...
    return run_experiment_rgb_gripper.Runner(base_dir, create_agent_fn)
  # Continuously runs training until max num_iterations is hit.
  elif FLAGS.schedule == 'continuous_train':
    return run_experiment_rgb_gripper.TrainRunner(base_dir, create_agent_fn)
  else:
...
def launch_experiment(create_runner_fn, create_agent_fn):
  ...
  Args:
    ...
    create_agent_fn: A function that takes as args a Tensorflow session and an
     Atari 2600 Gym environment, and returns an agent.
  """
  run_experiment_rgb_gripper.load_gin_configs(FLAGS.gin_files, FLAGS.gin_bindings)
