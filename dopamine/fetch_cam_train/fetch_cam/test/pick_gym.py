...
while not simple_policy.done:
    ...
    if ENV_RENDER:
        ...
        rgb_obs = env.env.sim.render(width=256, height=256, camera_name="external_camera_0", depth=False,
                mode='offscreen', device_id=-1)
        rgb_gripper = env.env.sim.render(width=256, height=256, camera_name="gripper_camera_rgb", depth=False,
            mode='offscreen', device_id=-1)
        # plt.figure(1)
        # plt.imshow(rgb_obs)
        plt.figure(1)
